...
export function signConnectionOAuthState(payload: ConnectionOAuthState) {
  const body = base64url(JSON.stringify(payload));
  const sig = base64url(crypto.createHmac("sha256", getSecret()).update(body).digest());
  return `${body}.${sig}`;
}

export function verifyConnectionOAuthState(token: string): ConnectionOAuthState | null {
  ...
  const parsed = JSON.parse(base64urlToBuffer(body).toString("utf8")) as ConnectionOAuthState;
  if (!parsed?.provider || !parsed?.orgId || !parsed?.exp) return null;
